...
For the experimental v8 production run, launch four independent processes with
`runtime.partition_count=4` and distinct `runtime.partition_index` values `0` through
`3`. A candidate belongs to exactly one partition, selected by SHA-256 of its private
source file ID; the ID is not logged. Each process derives a private
`partition-N` scratch directory, ledger, audit reservoir, and event log from the common
`scratch_root`. `programme_limit` and `source_file_id` retain their existing meanings
(the latter is processed only by its owning partition). Batch IDs and manifests include
the partition namespace, while programme-derived Parquet paths remain unchanged.

Preparation, download, decoding, segmentation, and OGG/Opus encoding run in parallel.
Each `publish_pending` call takes a single shared POSIX lock (configurable with
`runtime.publish_lock_path`, otherwise `<scratch_root>/publish.lock`) around Hub
inspection, commit, verification, and purge. The lock is a small local file and is
released if publication raises.
...
commit history, not from terminal scrollback alone. Emit metadata-only JSONL operational
logs with counts, timings, scratch use, ownership uncertainty, and rejection categories.
