...
counted as uncertainty; ASR agreement, WER, model-derived scores, and a large manual
quality threshold are not v8 acceptance gates. Model-backed alignment requirements
below apply only to explicitly inactive generic future infrastructure.
...
2. every open shard and configured pending shard in an unverified publication group;
   the default group retains up to 16 programmes and 96 shards;
...
`--max-scratch-bytes`, `--max-source-bytes`, `--shards-per-commit`, and the runtime-only
`programmes_per_commit` (16 in production). Also check Hub
access, target repository privacy, and source revision availability before processing.
The active v8 path is CPU/model-free and has no CUDA, GPU, model-cache, or model
alignment prerequisite.

Each partition process owns one programme at a time. Once all rows from a programme
are durably appended and fsynced to a recoverable local shard, record that state and
delete its source, decoded, and alignment temporary bytes. A lost or corrupt
uncommitted shard is regenerated by deterministic redownload from the pinned source.
Never retain source programmes until a multi-shard upload group completes, and never let
Datasets materialise the complete audio split or derived dataset. A group is durably
appended programme by programme, sealed at 16 programmes (or its shard cap), then
published with one manifest and one Hub commit. A tail group is flushed on partition
exhaustion.

For production, run one named tmux worker window and one command:
`uv run python src/scripts/run_p1_production.py`. The preset starts eight children;
override `supervisor.process_count`, `supervisor.max_attempts`,
`supervisor.retry_delay_seconds`, `supervisor.shutdown_grace_seconds`, or
`supervisor.run_root` with Hydra values. Do not launch one shell or tmux window per
partition. A candidate belongs to exactly one partition, selected by SHA-256 of its
private source file ID; the ID is not logged. Each child derives a private
`partition-N` scratch directory, rollback-journal SQLite ledger, audit reservoir, and
metadata event log from the common run root. `programme_limit` and `source_file_id`
retain their existing meanings (the latter is processed only by its owning partition).
Batch IDs and manifests include the partition namespace, while programme-derived
Parquet paths remain unchanged.

Preparation, download, decoding, segmentation, and timestamp-native OGG/Opus encoding
run in parallel. Each `publish_pending` call takes the one shared POSIX lock
(configurable with `runtime.publish_lock_path`, otherwise `<run_root>/publish.lock`)
around private Hub inspection, commit, verification, and purge. The parent writes
sanitised, non-interleaved status logs to `<run_root>/supervisor/partition-N.jsonl`
and aggregate `DONE`/`FAILED` markers to `<run_root>/supervisor/`; per-partition
markers are under `<run_root>/supervisor/markers/`. IPC contains status metadata only.
Never inspect live ledgers; use the marker and log files for parent-level operations,
and inspect a partition ledger only during an explicitly stopped recovery procedure.
...
- bind and validate the pipeline digest before selecting new programmes; an
  incompatible populated ledger is rejected without mutation. Any model construction
  is confined to inactive generic future infrastructure;
...
source and target-licence provenance. The dataset-licence template is the
...
- update the dataset card with final structural statistics and the aggregate report;
...
- Delete the v7 target cleanly and recreate it without archiving the v7 payload.
- Allocate fresh v8 scratch; do not reuse v7 staging, caches, or model artefacts.
...
cache behaviour are proven. For v8, this is a structural and model-free gate; CUDA,
GPU availability, model caches, and model alignment are not prerequisites.
...
### Phase 2: targeted v8 ownership pilot

Start from fresh v8 scratch and process a small, deterministic, stratified sample, not
the first rows in the stream. Target programmes and clips where speaker metadata is
incomplete so the best-effort ownership policy is exercised. Retain the source text
once, record uncertain ownership as aggregate metadata, and confirm that accepted
rows still have complete structural provenance.

This pilot is a targeted structural/ownership check, not a model-quality benchmark.
It must not require CUDA, a model cache, ASR WER, model-derived final metrics, or a
large manual audit threshold. A small manual spot check may document obvious source or
boundary defects, but it is informative rather than an acceptance criterion.

Tune only the versioned timestamp and structural thresholds, then rerun the pilot from
fresh scratch. The pilot gate is exact schema, immutable identities, audio encoding,
duration/bounds, deterministic text ownership, and truthful aggregate reporting.

### Phase 3: bounded production run

Run one visible, named session on the production host and check free disk before
launch. Do not stop or alter unrelated services without separate permission. Active v8
requires no CUDA device, GPU process, model cache, or model revision alignment.

Publish one bounded batch at a time. The Python parent reports progress through its
per-partition metadata-only JSONL logs and aggregate markers, not terminal scrollback
or live-ledger reads. Emit metadata-only operational logs with counts, timings, scratch
use, ownership uncertainty, and rejection categories.
...
Produce a remote-derived v8 report covering:

- accepted and rejected programmes and segments;
- total and per-show audio hours;
- duration, word-count, and source-timestamp distributions;
- coverage against usable transcript duration;
- rejection reasons, rates, and best-effort ownership counts;
- duplicate IDs and overlapping source intervals;
- speaker-count and programme-position breakdowns;
- exact schema, identity, encoding, and immutable publication checks.

Do not add ASR WER, model-derived final metrics, model alignment, or a manual-audit
threshold to the active v8 report. Those belong only in explicitly inactive generic
future infrastructure.

**Gate:** all structural checks pass, the model-free sanity gate validates its selected
v8 rows, and any source or ownership anomalies are truthfully reported. Relaxed v8
quality policy means that uncertain lexical ownership is counted, not silently treated
as a fatal quality failure.
...
this active v8 publication gate.

## Evidence to retain

Retain only for the active v8 dataset:

- evidence that the old v7 target was deleted and recreated cleanly without an archive;
- fresh v8 scratch provenance and code/configuration identity;
...
- aggregate structural reports, ownership uncertainty, and any manual spot-check notes;
- operational timings and peak scratch usage. Model caches, model revisions, ASR WER,
  and model-derived final metrics are not active v8 evidence.
